...
(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.Sunburst = factory());
  ...
  var sunburst = index$2({
    ...
    methods: {
      _parseData: function _parseData(state) {
        if (state.data) {
          var hierData = hierarchy(state.data, index$1(state.children)).sum(index$1(state.size));
          if (state.sort) {
          ...
          if (state.excludeRoot) {
            ...
            var yScale = linear().domain([hierData.y1 - hierData.y0, 1]);
    ...
    init: function init(domNode, state) {
      ...
      state.arc = d3Arc().startAngle(function (d) {
      ...
      state.canvas = state.svg.append('g').style('font-family', 'sans-serif').style('font-size', "".concat(state.fontSize, "px"));
      state.tooltip = index()(el);
    ...
    update: function update(state) {
      ...
      var nameOf = index$1(state.label);
      var colorOf = index$1(state.color);
      var strokeColorOf = index$1(state.strokeColor);
      var nodeClassNameOf = index$1(state.nodeClassName);
      ...
      state.svg.transition(transition$1).tween('scale', function () {
        var xd = d3Interpolate(state.angleScale.domain(), [focusD.x0, focusD.x1]);
        var yd = d3Interpolate(state.radiusScale.domain(), [focusD.y0, maxY]);
        return function (t) {
      ...
      // white contour
      newSlice.selectAll('.text-contour').style('stroke-width', "".concat(state.fontStrokeWidth, "px"));
      ...
      function angularTextFits(d) {
        return measureTextWidth(nameOf(d.data).toString(), state.fontSize, {
          strokeWidth: state.fontStrokeWidth
      ...
      function radialTextFits(d) {
        ...
        if (availableHeight < state.fontSize + state.fontStrokeWidth) return false; // not enough angular space
      ...
      function autoPickLabelOrientation(d) {
        ...
        if (!orientation) {
          ...
          if (availableArcWidth < state.fontSize + state.fontStrokeWidth) {
